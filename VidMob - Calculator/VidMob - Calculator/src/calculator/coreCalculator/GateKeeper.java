...
*Class Dependency - created by CalculatorCentral.java class and return the sanity check result back to CalculatorCentral class
*APIs/Libraries used - java.util.HashSet and Stack
*
*
//CONSTRUCTOR / METHODS INFO
*1. sanityCheck(String)
*Purpose - to perform a sanity check on user's string input; to perform 5 types (9 cases) of checks (case details are below.)
*Dependency	- called by CalculatorCentral class and return calculation result back to CalculatorCentral class
*Visibility - public
...
*Return type - boolean
*
//Gate Keeper Sanity Check Cases
Case #1: If the input string is null / empty
Case #2 - Unacceptable symbols (including letters, [], {}, @)
Case #3 - Operators related cases
Case #3.1 - If the first char is not '-' ('-' at index 0 is accepted.)
Case #3.2 - If there are more than two operators in a row
Case #3.3 - If there are two operators in a row and the second operator is not a '-'
Case #3.4 - If the starting 2 char are both '-'
Case #4 - Two decimal points next to each other
Case #5 - Valid Parentheses
Case #5.1 - Check for any empty '()'
Case #5.2 - Parentheses must be in pair
*/
